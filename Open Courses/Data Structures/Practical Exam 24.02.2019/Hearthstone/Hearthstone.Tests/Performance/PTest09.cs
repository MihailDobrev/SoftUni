...
public class PTest09
{
    ...
    [TestCase]
    public void Play_10_000_card_get_best_of_range()
    {
        ...
        stopwatch.Start();
        IEnumerable<Card> cards1 = this.board.GetBestInRange(0, int.MaxValue);
        stopwatch.Stop();
        long executionTimeInMillis = stopwatch.ElapsedMilliseconds;
        Assert.IsTrue(executionTimeInMillis <= 35);
     

        Assert.AreEqual(initialCount, cards1.Count());
    }
}
