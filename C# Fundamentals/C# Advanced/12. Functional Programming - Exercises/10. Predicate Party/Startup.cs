    ...
    public class Startup
    {
        public static void Main()
        {
            ...
            while (!comand.Equals("Party!"))
            {
                ...
                {
                    DoOperation(commandArgs[0], guests, n => n.StartsWith(commandArgs[2]));
                }
                else if (currentComand.Equals("EndsWith"))
                {
                    DoOperation(commandArgs[0], guests, n => n.EndsWith(commandArgs[2]));
                }
                else if (currentComand.Equals("Length"))
                {
                    DoOperation(commandArgs[0], guests, n => n.Length == int.Parse(commandArgs[2]));
                }
        ...
        private static void DoOperation(string command, List<string> guestList, Func<string, bool> condition)
        {
